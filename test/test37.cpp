...
{	return exp(-(sqr(_rX) + sqr(_rY) + sqr(_rZ)));
}
static constexpr auto X0 = 1.1;
static constexpr auto Y0 = 1.2;
static constexpr auto Z0 = 1.3;

BOOST_AUTO_TEST_CASE(taylor_37)
{	using namespace taylor;
	using namespace boost::mp11;
	const auto sMap = read("data37.txt");
...
BOOST_AUTO_TEST_CASE(jacobian_37)
{	using namespace jacobian;
	using namespace boost::mp11;
	const auto sMap = read("data37.txt", true);
