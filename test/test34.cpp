...
static auto black_scholes_option_price_call(
                                                            double K,
                                                            Price const& S,
                                                            Sigma const& sigma,
                                                            Tau const& tau,
...
BOOST_AUTO_TEST_CASE(taylor_34)
{	using namespace taylor;
	using namespace boost::mp11;
	const auto sMap = read("data34.txt");
	...
	const auto s5 = black_scholes_option_price_call(K, S, sigma, tau, r);
	__EQUAL_TAYLOR__();
}
BOOST_AUTO_TEST_CASE(jacobian_34)
{	using namespace jacobian;
	using namespace boost::mp11;
	const auto sMap = read("data34.txt", true);
	double const K = 100.0;  // Strike price.
	constexpr std::size_t MAX = 2;
