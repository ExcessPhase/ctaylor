	...
	else
{
  ...
  std::cout << std::setprecision(std::numeric_limits<double>::digits10)
            ...
            << "\n## First-order Greeks\n"
            << "autodiff call delta = " << call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<1> > >()) << '\n'
            << " formula call delta = " << formula_call_delta << '\n'
            << "autodiff call vega  = " << call_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<1> > >()) << '\n'
            << " formula call vega  = " << formula_vega << '\n'
            << "autodiff call theta = " << -call_price.getDer(mp_list<pair<mp_size_t<2>, mp_size_t<1> > >())
            << '\n'  // minus sign due to tau = T-time
            << " formula call theta = " << formula_call_theta << '\n'
            << "autodiff call rho   = " << call_price.getDer(mp_list<pair<mp_size_t<3>, mp_size_t<1> > >()) << '\n'
            << " formula call rho   = " << formula_call_rho << '\n'
            << '\n'
            << "autodiff put delta = " << put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<1> > >()) << '\n'
            << " formula put delta = " << formula_put_delta << '\n'
            << "autodiff put vega  = " << put_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<1> > >()) << '\n'
            << " formula put vega  = " << formula_vega << '\n'
            << "autodiff put theta = " << -put_price.getDer(mp_list<pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << " formula put theta = " << formula_put_theta << '\n'
            << "autodiff put rho   = " << put_price.getDer(mp_list<pair<mp_size_t<3>, mp_size_t<1> > >()) << '\n'
            << " formula put rho   = " << formula_put_rho << '\n'
            << "\n## Second-order Greeks\n"
            << "autodiff call gamma = " << call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<2> > >()) << '\n'
            << "autodiff put  gamma = " << put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<2> > >()) << '\n'
            << "      formula gamma = " << formula_gamma << '\n'
            << "autodiff call vanna = " << call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<1> >, pair<mp_size_t<1>, mp_size_t<1> > >()) << '\n'
            << "autodiff put  vanna = " << put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<1> >, pair<mp_size_t<1>, mp_size_t<1> > >()) << '\n'
            << "      formula vanna = " << formula_vanna << '\n'
            << "autodiff call charm = " << -call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<1> >, pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << "autodiff put  charm = " << -put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<1> >, pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << "      formula charm = " << formula_charm << '\n'
            << "autodiff call vomma = " << call_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<2> > >()) << '\n'
            << "autodiff put  vomma = " << put_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<2> > >()) << '\n'
            << "      formula vomma = " << formula_vomma << '\n'
            << "autodiff call veta = " << call_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<1> >, pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << "autodiff put  veta = " << put_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<1> >, pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << "      formula veta = " << formula_veta << '\n'
            << "\n## Third-order Greeks\n"
            << "autodiff call speed = " << call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<3> > >()) << '\n'
            << "autodiff put  speed = " << put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<3> > >()) << '\n'
            << "      formula speed = " << formula_speed << '\n'
            << "autodiff call zomma = " << call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<2> >, pair<mp_size_t<1>, mp_size_t<1> > >()) << '\n'
            << "autodiff put  zomma = " << put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<2> >, pair<mp_size_t<1>, mp_size_t<1> > >()) << '\n'
            << "      formula zomma = " << formula_zomma << '\n'
            << "autodiff call color = " << call_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<2> >, pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << "autodiff put  color = " << put_price.getDer(mp_list<pair<mp_size_t<0>, mp_size_t<2> >, pair<mp_size_t<2>, mp_size_t<1> > >()) << '\n'
            << "      formula color = " << formula_color << '\n'
            << "autodiff call ultima = " << call_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<3> > >()) << '\n'
            << "autodiff put  ultima = " << put_price.getDer(mp_list<pair<mp_size_t<1>, mp_size_t<3> > >()) << '\n'
